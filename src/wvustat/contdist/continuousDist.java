...
public class continuousDist extends JApplet implements Runnable,ActionListener{
	...
	public void init(){
		...
		try { 
		    distName = new String(getParameter("DistributionName"));
		} catch ( Exception e ) {distName = new String("Normal");}
		
		
		try { cInfo	= new continuousInfo(distName, getParameter("display")); 
		} catch (Exception e) { cInfo = new continuousInfo(distName, "PDF");}	
	
		
		if (cInfo.isNormal){
			try {
				cInfo.setInitPara(getParameter("mu"), (int)0);
				cInfo.setInitPara(getParameter("sd"), (int)1);
			} catch (Exception e) {
		...
		bottomPanel.add(textFieldPara1,new GridBagConstraints(2,0,1,1,2,0,GridBagConstraints.EAST,GridBagConstraints.HORIZONTAL,new Insets(2,2,2,2),0,0));
		...
		bottomPanel.add(textFieldPara2,new GridBagConstraints(2,1,1,1,2,0,GridBagConstraints.EAST,GridBagConstraints.HORIZONTAL,new Insets(2,2,2,2),0,0));
		}
		else {
		...
		bottomPanel.add(tmpLabelPara2,new GridBagConstraints(2,1,1,1,2,0,GridBagConstraints.EAST,GridBagConstraints.HORIZONTAL,new Insets(2,2,2,2),0,0));
		...
		try {
			qPanel = new quantilePanel(cInfo, getParameter("q"), this);
		} catch (Exception e) {
