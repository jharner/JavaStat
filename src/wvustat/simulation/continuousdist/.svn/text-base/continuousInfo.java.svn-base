...
public class continuousInfo {
  ...
  public void setInterval(double lowBd, double uppBd, int tmpType){
  	...
  	/*	if (intervalType == 4) {
   			tmpString = "  P( X = " + new Double(lowBd).toString() + " ) = 0.0";
   			approxProbString = "  P( Z = " + StandNormalString(lowBd, 6) + " ) = 0.0";
   		}
   		else { */
  ...
  public String StandNormalString(double v, int decimal) {
  		...
  		value = Math.round(value * Math.pow(10, decimal)) / Math.pow(10, decimal); //added by djluo
